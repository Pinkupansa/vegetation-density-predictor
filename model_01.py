...
m = 100      #nombre d'exemples dans la data
...
print(training_set_batched.element_spec)
model_history = unet.fit(training_set_batched, epochs=EPOCHS, use_multiprocessing='True')
unet.save("saved_model/unet02")
...
predictions = [unet.predict(tf.reshape(image, (1,256,256,1))) for image in images]
display([images,masks,predictions])
print(type(predictions[0]))
print(predictions[0].shape)
plt.imshow(tf.keras.utils.array_to_img(tf.reshape(predictions[0], (256, 256, 1))))
